...
if uploaded_files:
    ...
    # नया फीचर: Type of Return काउंटिंग बॉक्स
    courier_rto_count = df_all[df_all['Type of Return'] == 'Courier Return (RTO)'].shape[0]
    customer_return_count = df_all[df_all['Type of Return'] == 'Customer Return'].shape[0]
    col1, col2 = st.columns(2)
    col1.markdown(f"<div style='background-color: #d4edda; padding: 10px; border-radius: 5px; text-align: center;'>"
                  f"<h3 style='color: #155724;'>Courier Return (RTO)</h3><h1>{courier_rto_count}</h1></div>", unsafe_allow_html=True)
    col2.markdown(f"<div style='background-color: #f8d7da; padding: 10px; border-radius: 5px; text-align: center;'>"
                  f"<h3 style='color: #721c24;'>Customer Return</h3><h1>{customer_return_count}</h1></div>", unsafe_allow_html=True)
    ...
    # Separate tables: Courier Return (RTO) and Customer Return
