...
with col_od:
    if 'Order Date' in df.columns:
        all_dates_raw = df['Order Date'].dropna().astype(str)
        order_date_options = sorted(all_dates_raw.unique().tolist())
        sel_order_dates = multiselect_with_select_all('Order Date चुनें', options=order_date_options, default=[], key='od_ms')
    else:
...
st.success('तैयार: In-filter Select All (तीनों फ़िल्टर्स), Blank Packet Id, Order Date filter, सुरक्षित गेटिंग, और Grand Total (Master में single, Style-wise में right + bottom) Excel/PDF सहित लागू हो गए हैं।')
