# --------------------------------------------------------------
# 📦 Meesho Invoice Auto Sourcing Sorter (v8 - Page Range Display)
# --------------------------------------------------------------
# Required:
...
st.title("📦 Meesho Invoice Auto Sourcing – v8 (Page Range Display)")
st.caption("Features: Multi-PDF merge, custom style order, courier+style selection, page range display with print commands")
...
if uploaded_files:
    ...
    # ----------------------------------------------------
    # Per-Courier Selection & Page Range Display
    # ----------------------------------------------------
    st.subheader("🎯 Select Styles per Courier & View Page Ranges")
    ...
    for courier in COURIER_PRIORITY + ["UNKNOWN"]:
        ...
        with st.expander(f"🚚 {courier} — Select Styles"):
            ...
            # Show page ranges for each style
            st.markdown("**📄 Page Ranges for Styles:**")
            show_styles = selected if selected else ordered_styles
            for sty in show_styles:
                ...
                if pages:
                    ...
                    st.caption(f"**{sty}**: p. {human} | 🖨️ Print range: `{cmd}`")
    ...
    if combined_pages:
        combined_pages = sorted(set(combined_pages))
        cranges = page_ranges_from_list(combined_pages)
        chuman = format_ranges_human(cranges, one_indexed=True)
        ccmd = format_ranges_printer(cranges, one_indexed=True)
        st.info(f"📄 Combined page range: p. {chuman} | 🖨️ Print range: `{ccmd}`")
