import streamlit as st
import pandas as pd
from io import BytesIO
from fpdf import FPDF
import tempfile

st.set_page_config(page_title="Courier Partner Delivery & Return Analysis", layout="wide")
st.title("📦 Courier Partner Delivery & Return Analysis")

def add_grand_totals(df):
    df["Grand Total"] = df.sum(axis=1, numeric_only=True)
    total_row = df.sum(axis=0, numeric_only=True)
    total_row.name = "Grand Total"
    df = pd.concat([df, pd.DataFrame([total_row])], axis=0)
    return df

def pivot_to_pdf(pivot_df, title="Courier Partner Summary by Delivered Date"):
    max_cols = len(pivot_df.columns) + 1  # plus 1 for index
    orientation = 'P'
    pdf_width = 210
    if max_cols > 7:  # wide table: use landscape
        orientation = 'L'
        pdf_width = 297

    pdf = FPDF(orientation=orientation, unit='mm', format='A4')
    pdf.add_page()
    pdf.set_font("Arial", style='B', size=16)
    pdf.cell(0, 12, title, ln=1, align='C')
    pdf.set_font("Arial", size=9)

    col_names = [str(x) for x in [""] + list(pivot_df.columns)]
    data = [col_names]
    for idx, row in pivot_df.iterrows():
        data.append([str(idx)] + [str(x) for x in row])

    col_width = max(26, pdf_width // max_cols)

    for row in data:
        for value in row:
            pdf.cell(col_width, 8, str(value), border=1, align="C")
        pdf.ln()

    with tempfile.NamedTemporaryFile(delete=False, suffix=".pdf") as tmp_file:
        pdf.output(tmp_file.name)
        tmp_file.seek(0)
        pdf_bytes = tmp_file.read()

    return pdf_bytes

st.sidebar.header("Upload multiple files")
uploaded_files = st.sidebar.file_uploader(
    "Select CSV/XLSX/XLS files", type=["csv", "xlsx", "xls"], accept_multiple_files=True
)

HEADER_ROW_INDEX = 7

if uploaded_files:
    dfs = []
    for f in uploaded_files:
        df = pd.read_csv(f, skiprows=HEADER_ROW_INDEX) if f.name.endswith(".csv") else pd.read_excel(f, skiprows=HEADER_ROW_INDEX)
        dfs.append(df)
    df_all = pd.concat(dfs, ignore_index=True)

    # Clean columns
    if 'Courier Partner' in df_all.columns:
        df_all['Courier Partner'] = df_all['Courier Partner'].replace({'PocketShip': 'Valmo', 'Valmo': 'Valmo'})
    if 'Delivered Date' in df_all.columns:
        df_all['Delivered Date'] = pd.to_datetime(df_all['Delivered Date'], errors='coerce').dt.date

    # Filters
    st.sidebar.header("Filters")
    all_dates = sorted([str(d) for d in df_all['Delivered Date'].dropna().unique()])
    search_date = st.sidebar.text_input("Search Date (YYYY-MM-DD)", key="date_search")
    filtered_dates = [d for d in all_dates if search_date in d] if search_date else all_dates
    select_all_dates = st.sidebar.checkbox("Select all dates", value=True, key="selectalldates")
    selected_dates = filtered_dates if select_all_dates else st.sidebar.multiselect("Select Delivered Date(s)", filtered_dates, default=filtered_dates)

    all_couriers = sorted([str(c) for c in df_all['Courier Partner'].dropna().unique()])
    search_courier = st.sidebar.text_input("Search Courier", key="cour_search")
    filtered_couriers = [c for c in all_couriers if search_courier.lower() in c.lower()] if search_courier else all_couriers
    select_all_couriers = st.sidebar.checkbox("Select all couriers", value=True, key="selectallcour")
    selected_couriers = filtered_couriers if select_all_couriers else st.sidebar.multiselect("Select Courier(s)", filtered_couriers, default=filtered_couriers)

    all_skus = [str(s) for s in df_all['SKU'].dropna().unique()] if 'SKU' in df_all.columns else []
    search_sku = st.sidebar.text_input("Search SKU", key="sku_search")
    filtered_skus = [s for s in all_skus if search_sku.lower() in s.lower()] if search_sku else all_skus
    select_all_skus = st.sidebar.checkbox("Select all SKUs", value=True, key="selectallsku")
    selected_skus = filtered_skus if select_all_skus else st.sidebar.multiselect("Select SKU(s)", filtered_skus, default=filtered_skus)

    # Apply filters
    filtered_df = df_all.copy()
    if selected_dates:
        filtered_df = filtered_df[filtered_df['Delivered Date'].astype(str).isin(selected_dates)]
    if selected_couriers:
        filtered_df = filtered_df[filtered_df['Courier Partner'].isin(selected_couriers)]
    if selected_skus:
        filtered_df = filtered_df[filtered_df['SKU'].isin(selected_skus)]

    # Main table and preview
    if {'Delivered Date', 'Courier Partner'}.issubset(filtered_df.columns):
        summary = filtered_df.groupby(['Delivered Date', 'Courier Partner']).size().reset_index(name="Total Packets")
        pivot_df = summary.pivot_table(index="Delivered Date", columns="Courier Partner", values="Total Packets", aggfunc="sum", fill_value=0)
        pivot_df = add_grand_totals(pivot_df)
        st.subheader("Courier Partner Summary by Delivered Date")
        st.dataframe(pivot_df, use_container_width=True)

        pdf_bytes = pivot_to_pdf(pivot_df, title="Courier Partner Summary by Delivered Date")
        st.download_button("Download Courier Partner Table (PDF, Portrait/Landscape)", pdf_bytes, file_name="courier_partner_summary.pdf", mime="application/pdf")

    # SKU-wise Reason Summary
    if {'SKU', 'Detailed Return Reason'}.issubset(filtered_df.columns):
        st.subheader("SKU-wise Return Reason Summary")
        reason_summary = filtered_df.groupby(['SKU','Detailed Return Reason']).size().reset_index(name='Return Count')
        reason_pivot = reason_summary.pivot_table(index='SKU', columns='Detailed Return Reason', values='Return Count', fill_value=0)
        reason_pivot = add_grand_totals(reason_pivot)
        st.dataframe(reason_pivot, use_container_width=True)

    # Style group keyword
    st.subheader("Style Group Reason Summary (by keyword)")
    style_group_key = st.text_input("Enter Style Group keyword (e.g. 'POCKET TIE')")
    if style_group_key:
        filtered_df['Style Group'] = filtered_df['SKU'].apply(lambda x: style_group_key if style_group_key.lower() in str(x).lower() else None)
        group_df = filtered_df[filtered_df['Style Group'].notna()]
        if not group_df.empty:
            g_summary = group_df.groupby(['Style Group','Detailed Return Reason']).size().reset_index(name='Return Count')
            group_pivot = g_summary.pivot_table(index='Style Group', columns='Detailed Return Reason', values='Return Count', fill_value=0)
            group_pivot = add_grand_totals(group_pivot)
            st.dataframe(group_pivot, use_container_width=True)
        else:
            st.info("No SKUs found matching that style group.")

    # Download buttons
    st.subheader("Download Options")
    csv_master = df_all.to_csv(index=False).encode("utf-8")
    st.download_button("Download All Data (CSV)", csv_master, "all_data.csv", "text/csv")
    csv_filtered = filtered_df.to_csv(index=False).encode("utf-8")
    st.download_button("Download Filtered Data (CSV)", csv_filtered, "filtered_data.csv", "text/csv")
    excel_buffer = BytesIO()
    with pd.ExcelWriter(excel_buffer, engine='xlsxwriter') as writer:
        df_all.to_excel(writer, index=False, sheet_name="All Data")
        filtered_df.to_excel(writer, index=False, sheet_name="Filtered Data")
        if 'pivot_df' in locals():
            pivot_df.to_excel(writer, sheet_name="Courier Summary")
        if 'reason_pivot' in locals():
            reason_pivot.to_excel(writer, sheet_name="Return Reason Summary")
        if 'group_pivot' in locals():
            group_pivot.to_excel(writer, sheet_name="Style Group Summary")
    st.download_button("Download Excel (All Summaries)", excel_buffer.getvalue(), "courier_return_full.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")

else:
    st.info("Please upload at least one CSV or Excel file.")
