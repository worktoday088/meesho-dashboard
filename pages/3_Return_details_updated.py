...
if stylegroup_key and "SKU" in df_style.columns:
    ...
    # Accept either 'Variation' or 'Var' or 'Product Variation' as common names
    var_cols_candidates = [c for c in group_df.columns if c.lower() in ("variation", "var", "product variation")]
    var_col = var_cols_candidates[0] if var_cols_candidates else "Variation"

    required_cols = {"Detailed Return Reason", "Qty"}
    # Variation is optional but preferred
    if var_col not in group_df.columns:
        # If Variation missing, we'll create a placeholder column so grouping still works
    ...
    if not required_cols.issubset(group_df.columns):
        ...
        # ------------------------------
        # PDF Pivot: index=Detailed Return Reason, columns=Variation
        # ------------------------------
        try:
            ...
            # fallback to simpler pivot if unexpected issues
            pivot_pdf_df = groupsummary.pivot_table(
                index="Detailed Return Reason",
                values="Return Count",
                aggfunc="sum",
    ...
    if stylegroup_key:
        st.info("Style Group requires SKU, Detailed Return Reason and Qty columns. Variation column is optional.")
